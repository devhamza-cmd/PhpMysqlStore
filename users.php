...
<body>
    ...
    <div class="d-flex">
        ...
        <div class="two">
            ...
            <div class="d-flex  dinfo">
                <div class=" partone ">
                    ...
                    <div class="moneydiv">
                        ...
                        <h4>
                            <?php
                            $sql = "SELECT ROUND((COUNT(id)/12),2) as 'total' FROM users where role='u' ";
                            $table = $pdo->query($sql);
                            while ($row = $table->fetch(PDO::FETCH_ASSOC)) {
                                $taotal = $row['total'];
                    ...
                    <div class="moneydiv">
                        ...
                        <h4>
                            <?php
                            $sql = "SELECT count(DISTINCT id) as 'total' FROM users u,commande c WHERE u.id NOT IN(SELECT DISTINCT c.codeu from  commande c) and role='u'";
                            $table = $pdo->query($sql);
                            while ($row = $table->fetch(PDO::FETCH_ASSOC)) {
                                $taotal = $row['total'];
                    ...
                    <div class="moneydiv">
                        ...
                        <h4>
                            <?php
                            $sql = "SELECT MONTHNAME(date) ,count(id) from users where role='u' GROUP by month(date) ORDER BY `count(id)` DESC limit 1";
                            $table = $pdo->query($sql);
                            while ($row = $table->fetch(PDO::FETCH_ASSOC)) {
            ...
            <div class="cyear">
                <?php
                $sql = "SELECT month(date) ,count(id) from users where role='u' GROUP by month(date) ORDER BY `count(id)` DESC limit 1
...
";
                ...
                <div class="year">
                    ...
                    for ($i = 1; $i < 13; $i++) {
                        if ($i < 10) {
                            $sql = "SELECT count(id) from users where role='u' and date(date) BETWEEN '2023/0$i/01' and '2023/0$i/30' GROUP by month(date)  ORDER BY `count(id)` DESC limit 1";
                        } else {
                            $sql = "SELECT count(id) from users where role='u' and date(date) BETWEEN '2023/$i/01' and '2023/$i/30' GROUP by month(date)  ORDER BY `count(id)` DESC limit 1
";
                        }
                        $x = 0;
                        $table = $pdo->query($sql);
